...
// Define SDK options
const sdkOptions = [
  { name: "Java", icon: JavaIcon, sdk: "java" },
  { name: "Kotlin", icon: KotlinIcon, sdk: "kotlin" },
  { name: "Scala", icon: ScalaIcon, sdk: "scala" },
  { name: "C++", icon: CppIcon, sdk: "cpp" },
  { name: ".NET", icon: DotNetIcon, sdk: "dotnet" },
  { name: "Go", icon: GoIcon, sdk: "go" },
  { name: "Python", icon: PythonIcon, sdk: "python" },
  { name: "Node", icon: NodeIcon, sdk: "node" },
  { name: "Ruby", icon: RubyIcon, sdk: "ruby" },
  { name: "Rust", icon: RustIcon, sdk: "rust" },
]
...
export default function Sidebar({
  ...
  onSdkChange,
  mode = "performance",
  selectedSituationalSdk = "",
  onSituationalSdkChange,
  onModeChange,
}: SidebarProps) {
  ...
  useEffect(() => {
    setLocalSelectedSdk(selectedSituationalSdk)
  }, [selectedSituationalSdk])

  const isActive = (sdk: string) => sdk === activeSdk
  const isSituationalSdkActive = (sdk: string) => sdk === localSelectedSdk

  // SDK state is driven by props from app-layout (synced from URL params)

  const handleSdkClick = (sdk: string) => {
    console.log('Sidebar: handleSdkClick called with SDK:', sdk)
    if (onSdkChange) {
      onSdkChange(sdk)
      console.log('Sidebar: called onSdkChange with:', sdk)
    } else {
      console.log('Sidebar: onSdkChange is not defined!')
    }
  }

  const handleSituationalSdkClick = (sdk: string) => {
    setLocalSelectedSdk(sdk)
    // Navigation is handled by app-layout's handleSituationalSdkChange
    if (onSituationalSdkChange) {
      onSituationalSdkChange(sdk)
    }
  }

  const handleModeChange = (value: string) => {
    const newMode = value as "performance" | "situational"
  ...
  return (
    <div className="w-64 border-r h-screen bg-white dark:bg-slate-900 flex flex-col">
      <div className="p-6 border-b">
        <div className="flex items-center gap-2 font-semibold mb-4">
          <BarChart3 className="h-5 w-5 text-primary" />
          <span className="text-lg">SDK Analytics</span>
        </div>

        {/* Mode toggle - now only Performance and Situational */}
      ...
      <div className="flex-1 overflow-auto py-4">
        {mode === "performance" ? (
          // Performance mode sidebar content
          <>
            <div className="px-3">
              <h3 className="text-xs font-medium text-muted-foreground mb-2 px-3">SDKs</h3>
              <nav className="space-y-1">
                {sdkOptions.map((item) => (
                  <div
                    ...
                    onClick={() => handleSdkClick(item.sdk)}
                  >
                    <item.icon
                      className={cn("h-4 w-4", isActive(item.sdk) ? "text-primary" : "text-muted-foreground")}
                    />
                    <span>{item.name}</span>
                    {isActive(item.sdk) && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-primary"></span>}
                  </div>
                ))}
              </nav>
            </div>
          ...
          // Situational mode sidebar content
          <>
            <div className="px-3">
              <h3 className="text-xs font-medium text-muted-foreground mb-2 px-3">Filter by SDK</h3>
              <nav className="space-y-1">
                <div
                  className={cn(
                    "flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors cursor-pointer",
                  ...
                  onClick={() => handleSituationalSdkClick("")}
                >
                  <CheckCircle2 className={cn("h-4 w-4", !localSelectedSdk ? "text-primary" : "text-muted-foreground")} />
                  <span>All SDKs</span>
                  {!localSelectedSdk && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-primary"></span>}
                </div>
                ...
                {sdkOptions.map((item) => (
                  <div
                    ...
                    className={cn(
                      "flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors cursor-pointer",
                      isSituationalSdkActive(item.sdk)
                        ? "bg-primary/10 text-primary font-medium"
                        : "hover:bg-slate-100 dark:hover:bg-slate-800",
                    )}
                    onClick={() => handleSituationalSdkClick(item.sdk)}
                  >
                    <item.icon
                      className={cn(
                        "h-4 w-4",
                        isSituationalSdkActive(item.sdk) ? "text-primary" : "text-muted-foreground",
                      )}
                    />
                    <span>{item.name}</span>
                    {isSituationalSdkActive(item.sdk) && (
